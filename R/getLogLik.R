...
#' 
#' @title Calculate log likelihood state distribution.
#' 
...
#' 
#' @return The log likelihood of the observations sequences, given the model.
#' @usage getLogLik(hmm, obs = list(), emissionProbs = list(), dirFlags = list(), verbose = FALSE, nCores = 1, sizeFactors=matrix(1, nrow=length(obs), ncol=ncol(obs[[1]])))#' 
...
#' 
#' @export getLogLik
getLogLik = function(hmm, obs = list(), emissionProbs = list(), dirFlags = list(), 
    ...
    myLogLik = .Call("RGETLOGLIK", SEXPobs = obs, SEXPpi = initProb, SEXPA = transMat, 
        SEXPemission = emissionParams, SEXPtype = as.character(emission@type), 
